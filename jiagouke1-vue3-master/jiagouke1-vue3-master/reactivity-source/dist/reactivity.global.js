var VueReactivity = (function (exports) {
  ...
  function trigger(target, type, key, newValue, oldValue, oldTarget) {
      ...
      if (type === "clear" /* CLEAR */) { // 是清空，则调用所有effect
  ...
  function createGetter(isReadonly = false, shallow = false) {
      return function get(target, key, receiver) {
          if (key === "__v_isReactive" /* IS_REACTIVE */) { // 用来判断这个对象是reactive还是readonly
              return !isReadonly;
          }
          else if (key === "__v_isReadonly" /* IS_READONLY */) { // 如果取的是IS_READONLY 
              return isReadonly;
          }
          ...
          if ( // 是内置symbol  或者是原型链 查找到的，直接返回， 不需要收集symbol 和 __proto__的依赖
          isSymbol(key)
  ...
  function createSetter(shallow = false) {
      return function set(target, key, value, receiver) {
          ...
          if (!shallow) { // 对象被深层代理了 reactive({r:1})  proxy.r = reactive({a:1})
  ...
  function createInstrumentationGetter(isReadonly, shallow) {
      ...
      return (target, key, receiver) => {
          if (key === "__v_isReactive" /* IS_REACTIVE */) { // 处理 isReactive  isReadonly  toRaw
              return !isReadonly;
          }
  ...
  function createReactiveObject(target, isReadonly, baseHandlers, collectionHandlers) {
      ...
      if (targetType === 0 /* INVALID */) { // 不可扩展直接返回
          return target;
      }
