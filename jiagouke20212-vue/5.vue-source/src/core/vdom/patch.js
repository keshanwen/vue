...
export function createPatchFunction (backend) {
  ...
  function patchVnode ( // 两个节点的比较
    ...
    if (oldVnode === vnode) { // 新的和老的一样 就不用比对了
      return
    }
    ...
    const elm = vnode.elm = oldVnode.elm // 节点的拷贝
    ...
    if (isDef(data) && isPatchable(vnode)) { // 调用更新逻辑 包含 当前样式更新， 指令的更新
    ...
    if (isUndef(vnode.text)) { // 如果不是文本 并且两个人都有内容
      if (isDef(oldCh) && isDef(ch)) { // 两个儿子的比对
        ...
        if (isDef(oldVnode.text)) nodeOps.setTextContent(elm, '') // 将文本清空
        // 添加儿子
  ...
  return function patch (oldVnode, vnode, hydrating, removeOnly) {
    ...
    if (isUndef(oldVnode)) { // patch(vnode); // 组件的挂载
